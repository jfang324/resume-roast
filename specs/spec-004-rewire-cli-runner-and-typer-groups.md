...
- [ ] All tests pass (`make test`), with `tests/test_cli.py` changed only by
      the `app`→`cli` symbol rename.
- [ ] `git diff` on `tests/test_cli.py` shows only the import line and
...
- [ ] `test_config_group_shows_help_without_subcommand` and
      `test_show_group_shows_help_without_subcommand` pass — the explicit guard
      that both group Typers kept `no_args_is_help=True`.
- [ ] Manual smoke test, both entry points (`resume-roast ...` and
      ...
      show group help. (The `config credentials` save path may only be
      verifiable through the test suite on Windows — hidden-input prompts read
      the console API directly and cannot be driven by piped stdin — same
      pre-existing limitation noted in SPEC-002/003.)
- [ ] `poetry run resume-roast` (bare, via `[project.scripts]`) still resolves
      and shows help against the `:cli` entry point.
- [ ] INV-001 has passing enforcement tests
