...
def guarded(handler: Handler) -> Handler:
    """Wrap a handler so a domain error becomes a one-line error + exit 1.

    Centralizes the "domain failure -> user-facing error" boundary here
    ...
    def wrapper(*args: Any, **kwargs: Any) -> None:
        try:
            handler(*args, **kwargs)
        except (PersistenceError, ExtractionError, ApiError) as exc:
